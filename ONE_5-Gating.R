...
sampleNames(fs)<-unlist(lapply(fcs.path, function(x) return(gsub("/","-",unlist(strsplit(unlist(strsplit(x,"_5/"))[2]," "))[1]))))
...
if(centre=="Navios")
{
  comp.path <- list.files("/mnt/f/FCS data/OneStudy/Navios/COMP/",pattern="*B cells.csv",full.names = T)
...
if(centre=="Fortessa")
{
  comp.path <- "/mnt/f/FCS data/OneStudy/Fortessa/COMP/2018Jun13 Fortessa adj-Bcell.csv"
  comp<- read.csv(comp.path,check.names = F)
  comp <- comp[,-1]
  fs <- fsApply(fs, function(x) return(compensate(x, comp)))
}
...
PBMC<- fsApply_pb(fs.45, function(x){
  ...
  if (length(which(all.gates<main.p))>0 & centre %in% c("Navios","Fortessa"))
  {
