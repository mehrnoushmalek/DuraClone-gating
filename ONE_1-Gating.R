...
#Date: Last revised in July 2018
...
if(centre=="Navios")
{
  comp.path <- list.files("/mnt/f/FCS data/OneStudy/Navios/COMP",pattern="*Basic.csv",full.names = T)
  comp<- read.csv(comp.path,check.names = F)
  comp <- comp[,-1]
  fs <- fsApply(fs, function(x) return(compensate(x, comp)))
}
if(centre=="Fortessa")
{
  comp.path <- "/mnt/f/FCS data/OneStudy/Fortessa/COMP/2018Jun13 Fortessa adjBasic.csv"
  comp<- read.csv(comp.path,check.names = F)
  comp <- comp[,-1]
  fs <- fsApply(fs, function(x) return(compensate(x, comp)))
}
...
#                      Gating CD45
#*****************************************************************************************
if (centre %in% c("Navios","Fortessa"))
{
...
#                      Gating Lymph
#*****************************************************************************************
multi <- ifelse(type=="PBMCs",yes = 2,no=ifelse(centre=="German",yes = 1.6,no = 1.2 ))
...
#                      Gating CD16/64
#*****************************************************************************************
if (centre %in% c("DuraClone-T1D","German","CNTRP","CNTRP-PhaseII"))
{
...
write.csv(cell.prop,paste0(res.path,"/",centre,"/Results/", type,"/",panel,"/Proportions_",format(Sys.time(), "%Y-%m-%d"), ".csv"))
